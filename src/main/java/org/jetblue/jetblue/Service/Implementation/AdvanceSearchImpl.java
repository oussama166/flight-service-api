...
import org.jetblue.jetblue.Repositories.Specifications.FlightSpecification;
...
public class AdvanceSearchImpl implements AdvanceSearchService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
      AdvanceSearchImpl.class);

  @Override
  public List<FlightResponse> searchFlights(SearchRequest searchRequest) {
    System.out.println(
        "Searching flights with parameters: " +
            "departureTime=" +
    ...
    var spec = FlightSpecification.buildSpecification(
        searchRequest.departureTime(),
        searchRequest.arrivalTime(),
        searchRequest.origin(),
        searchRequest.departureCountry(),
        searchRequest.destination(),
        searchRequest.arrivalCountry(),
        searchRequest.flightStatus());

    return flightRepo
        .findAll(spec, Sort.by("price", "departureTime"))
        .stream()
        .map(FlightMapper::toFlightResponse)
        .collect(Collectors.toList());
  }

  public List<FlightItinerary> searchItinerary(
      String origin,
      String destination,
  ...
  private List<FlightItinerary> searchItineraryRecursive(
      String origin,
      String destination,
      LocalDateTime departureDate,
      int maxLegs) {
    ...
    if (maxLegs == 3) {
      List<Flight> directFlights = flightRepo.findFlightsByDeparture_LocationAndArrival_Location(
          origin,
          destination);
    ...
    System.out.println(
        String.format(
            "Searching for flights departing from %s after %s",
            origin,
            departureDate));
    List<Flight> firstLegs = flightRepo.findFlightsByDeparture_locationAndDepartureTimeAfter(
        origin,
        departureDate);
    ...
    for (Flight firstLeg : firstLegs) {
      ...
      if (firstLeg.getArrival().getLocation().equals(destination)) {
        ...
        System.out.println(
            "Arrival time of first leg: " + firstLeg.getArrivalTime());
        ...
        LocalDateTime nextDepartureDate = firstLeg
            .getArrivalTime()
            .plusHours(2);
        System.out.println(
            "Next departure search starts from: " + nextDepartureDate);
    ...
    System.out.println(
        String.format(
            "Returning %d itineraries from origin %s",
            results.size(),
            origin));
    return results;
  }

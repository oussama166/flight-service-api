...
public class DocumentImpl implements DocumentService {
    ...
    public DocumentImpl(DocumentRepo documentRepo, PassengerRepo passengerRepo) {
    ...
    @Override
    @Transactional
    public DocumentResponse uploadDocument(Long passengerId, DocumentType documentType, MultipartFile file)
            throws IOException {

        // 1. Find the passenger
        // Passenger passenger = passengerRepo.findById(passengerId)
        // .orElseThrow(() -> new IllegalArgumentException("Passenger not found with ID:
        // " + passengerId));
